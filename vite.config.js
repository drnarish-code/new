...
```react:PKD Pekan QMS:src/App.jsx
          ...
          <div className="flex-1 overflow-hidden flex flex-col bg-slate-950">
            {recentCalls.length === 0 ? (
              ...
              recentCalls.map((call, index) => (
                <div 
                  key={call.room} 
                  className={`grid grid-cols-2 py-10 px-8 border-b border-slate-800 items-center transition-all duration-700 ${
                    index === 0 
                ...
                >
                  ...
                  <div className={`text-7xl font-extrabold text-right tracking-wider tabular-nums transition-colors duration-700 ${
                    index === 0 
                          ...
                          <tr key={u.uid} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                            ...
                            <td className="p-3">
                              <select
                                ...
                                className={`text-xs font-bold py-1.5 px-3 rounded-full border outline-none bg-white text-slate-800 ${
                    ...
                    {Object.keys(hierarchy).map(stateName => (
                      <div 
                        ...
                        className={`p-3 border rounded-xl flex justify-between items-center cursor-pointer transition-all ${
                          adminSelectedState === stateName ? 'bg-purple-100 border-purple-500 shadow-sm' : 'bg-white hover:bg-slate-100'
                        }`}
                      >
                      ...
                      <div className="flex-1 overflow-y-auto space-y-2">
                        {Object.keys(hierarchy[adminSelectedState] || {}).map(distName => (
                          <div 
                            ...
                            className={`p-3 border rounded-xl flex justify-between items-center cursor-pointer transition-all ${
                              adminSelectedDistrict === distName ? 'bg-blue-100 border-blue-500 shadow-sm' : 'bg-white hover:bg-slate-100'
                            }`}
                          >
...
Apply these core adjustments on your computer and push the changes:

```bash
git add vite.config.js src/App.jsx
git commit -m "Configure direct HTML asset inliner and purge ES2018 operators for Sraf TV Browser compliance"
git push
```

Now launch the Sraf Browser on your TV and type in your web app's URL. The loading sequence will bypass any module or optional chaining checks, rendering the complete PKD Pekan QMS platform beautifully! Let me know when you've done the push!